  ...
  for (const p of params || []) {
    if (p.type === "checkbox") {
      result.push({
        ...
        value: args.includes(p.flag) ? "yes" : "no",
      });
    } else if (p.type === "multiselect") {
      const vals = [];
      args.forEach((a, i) => {
        if (a === p.flag && i + 1 < args.length) vals.push(args[i + 1]);
      });
      if (vals.length > 0)
        result.push({ label: p.label || p.name, value: vals.join(", ") });
    } else {
      const idx = args.indexOf(p.flag);
      if (idx !== -1 && idx + 1 < args.length)
        result.push({ label: p.label || p.name, value: args[idx + 1] });
    }
  }
  return result;
}
...
export default function BuildView() {
  ...
  useEffect(() => {
    ...
    fetchBranches(activeProject)
      .then((r) => {
        ...
        if (!branchStates[activeProject])
          setBranchStates((p) => ({
            ...p,
            [activeProject]: r.branches?.[0] || "",
          }));
      })
      .catch(() => {})
  ...
  const setBranch = (value) =>
    setBranchStates((prev) => ({
      ...prev,
      [activeProject]: typeof value === "string" ? value.trim() : value,
    }));
  ...
  const parsedCommits = (() => {
    try {
      return JSON.parse(selectedRun?.commits_json || "[]");
    } catch {
      return [];
    }
  })();
  ...
  if (projectKeys.length === 0)
    return (
      <div
        ...
        style={{ padding: 24, color: "var(--text-dim)" }}
      >
        No build projects configured. Add projects in Settings → Build Projects.
      </div>
    );
    ...
    <div className="build-view">
      ...
      <div className="build-form">
        ...
        {runs.length > 0 && (
          <div style={{ marginTop: 20 }}>
            <div className="build-recent-label" style={{ marginBottom: 6 }}>
              Runs
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              {runs.map((run) => {
                ...
                const queuePos =
                  run.status === "queued"
                    ? queuedRuns.findIndex((r) => r.id === run.id) + 1
                    : null;
                return (
                  <button
                    key={run.id}
      ...
      <div className="build-output">
        <div className="build-output-header">
          <span>
            {selectedRun
              ? `#${selectedRun.build_number} — ${selectedRun.type}`
              : "Output"}
            {selectedRun?.started_at && (
              ...
              >
                {formatRunDate(selectedRun.started_at)}
              </span>
            )}
          </span>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {isSelectedActive && (
              <>
                <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  ...
                  <span style={{ color: "var(--green)", letterSpacing: 0 }}>
                    {selectedRun?.status}
                  </span>
                </span>
                <button
            ...
            {liveStatus && !isSelectedActive && (
              <span style={{ color: statusColor(liveStatus), fontSize: 12 }}>
